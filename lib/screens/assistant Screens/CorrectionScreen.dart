// ignore_for_file: library_private_types_in_public_api, deprecated_member_use, unnecessary_new, unused_local_variable, non_constant_identifier_names, prefer_interpolation_to_compose_strings, avoid_print
...
class CorrectionScreen extends StatefulWidget {
  const CorrectionScreen({Key? key}) : super(key: key);
...
class _CorrectionScreenState extends State<CorrectionScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      home:  FlutterPainterExample(image_name:arguments['image_name']),
    );
  }
}

class FlutterPainterExample extends StatefulWidget {
   FlutterPainterExample({Key? key,  required this.image_name}) : super(key: key);
...
class _FlutterPainterExampleState extends State<FlutterPainterExample> {
  ...
  ui.ImageByteFormat? imageInByte;
  ...
  void initBackground() async {
    ...
    final image = await  NetworkImage('https://elitestudents.link/wp-content/uploads/'+widget.image_name).image; ////
                        ...
                        child: Column(
                          ...
                          children: [
                            if (controller.freeStyleMode !=
                              ...
                              Row(
                                children: [
                                  const Expanded(flex: 1, child: Text("السمك")),
                                  Expanded(
                                    flex: 3,
                            ...
                            if (controller.shapeFactory != null) ...[
                              const Divider(),
                              const Text("التحكم في الشكل"),

                              // Control text color hue
                              Row(
                                children: [
                                  const Expanded(
                                      flex: 1, child: Text("سمك الشكل")),
                                  Expanded(
                                    flex: 3,
                                    child: Slider.adaptive(
                                        min: 2,
              ...
              if (controller.shapeFactory == null)
                PopupMenuButton<ShapeFactory?>(
                  ...
                  itemBuilder: (context) => <ShapeFactory, String>{
                    LineFactory(): "خط مستقيم",
                    ArrowFactory(): "سهم",
                    DoubleArrowFactory(): " سهم مزدوج",
                    RectangleFactory(): "مستطيل",
                    OvalFactory(): "دائرة",
                  }
                      .entries
                      .map((e) => PopupMenuItem(
  ...
  void renderAndDisplayImage() async {
    ...
    final imageFuture = controller
        ...
        .then((ui.Image image) => image.pngBytes);

    final imageFile = controller
        .renderImage(backgroundImageSize)
        .then((ui.Image image) => image.pngBytes);

    final path = '${(await getTemporaryDirectory()).path}/img.jpg';

    await Dio().download('https://elitestudents.link/wp-content/uploads/' +widget.image_name, path);
    ...
    // For example:
    // ```dart
    // final file = '${(await getTemporaryDirectory()).path}/img.png';
...
class RenderedImageDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: const Text("ارسال التصحيح"),
